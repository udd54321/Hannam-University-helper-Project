...
const ThirdFloorScreen = () => {
  ...
  const [currentImage] = useState(floors['09_9F'].image);
  ...
  const setStartPointer = (room) => {
    navigation.navigate('Gil', { roomId: room, startFloor: '09_9F', goalFloor: '09_9F' }); // startFloor와 goalFloor 전달
  };

  const setArrivalPointer = (room) => {
    navigation.navigate('Gil', { roomId: room, startFloor: '09_9F', goalFloor: '09_9F' });
  };
const excludedRooms = ['099FS1', '099FS2', '099FS3'];
  return (
    <SafeAreaProvider>
      <SafeAreaView style={styles.container}>
        ...
        >
          ...
          >
            ...
            {Object.keys(floors['09_9F'].rooms)
                           .filter((roomId) => !excludedRooms.includes(roomId)) // 제외할 방 필터링
                           .map((roomId) => {
              const room = floors['09_9F'].rooms[roomId];
              const isRotated = [
                '090915',
                '090914',
                '090911',
