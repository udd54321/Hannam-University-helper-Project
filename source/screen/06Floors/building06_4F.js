...
const FourthFloorScreen = () => {
  ...
  const [currentImage] = useState(floors['06_4F'].image);
  ...
  const setStartPointer = (room) => {
    navigation.navigate('Gil', { roomId: room, startFloor: '06_4F', goalFloor: '06_4F' }); // startFloor와 goalFloor 전달
  };

  const setArrivalPointer = (room) => {
    navigation.navigate('Gil', { roomId: room, startFloor: '06_4F', goalFloor: '06_4F' });
  };
const excludedRooms = ['064FS1', '064FS2'];
  return (
    <SafeAreaProvider>
      <SafeAreaView style={styles.container}>
        ...
        >
          ...
          >
            ...
            {Object.keys(floors['06_4F'].rooms)
                           .filter((roomId) => !excludedRooms.includes(roomId)) // 제외할 방 필터링
                           .map((roomId) => {
              const room = floors['06_4F'].rooms[roomId];
              const isRotated = [
                '060402',
        '060403',
        '060404',
