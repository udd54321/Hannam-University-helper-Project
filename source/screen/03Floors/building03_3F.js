...
const ThirdFloorScreen = () => {
  ...
  const [currentImage] = useState(floors['03_3F'].image);
  ...
  const setStartPointer = (room) => {
    navigation.navigate('Gil', { roomId: room, startFloor: '03_3F', goalFloor: '03_3F' }); // startFloor와 goalFloor 전달
  };

  const setArrivalPointer = (room) => {
    navigation.navigate('Gil', { roomId: room, startFloor: '03_3F', goalFloor: '03_3F' });
  };
const excludedRooms = ['033FS1', '033FS2'];
  return (
    <SafeAreaProvider>
      <SafeAreaView style={styles.container}>
        ...
        >
          ...
          >
            ...
            {Object.keys(floors['03_3F'].rooms)
                           .filter((roomId) => !excludedRooms.includes(roomId)) // 제외할 방 필터링
                           .map((roomId) => {
              const room = floors['03_3F'].rooms[roomId];
              const isRotated = [
                '030301',
                '030302',
                '030303',
