...
import floors from './building11Floor';
...
const SecondFloorScreen = () => {
  ...
  const [currentImage] = useState(floors['11_2F'].image);
  ...
  const setStartPointer = (room) => {
    navigation.navigate('Gil', { roomId: room, startFloor: '11_2F', goalFloor: '11_2F' }); // startFloor와 goalFloor 전달
  };

  const setArrivalPointer = (room) => {
    navigation.navigate('Gil', { roomId: room, startFloor: '11_2F', goalFloor: '11_2F' });
  };
const excludedRooms = ['112FS1', '112FS2'];
  return (
    <SafeAreaProvider>
      <SafeAreaView style={styles.container}>
        ...
        >
          ...
          >
            ...
            {Object.keys(floors['11_2F'].rooms)
                           .filter((roomId) => !excludedRooms.includes(roomId)) // 제외할 방 필터링
                           .map((roomId) => {
              const room = floors['11_2F'].rooms[roomId];
              const isRotated = [
                '020101',
                '020101-A',
                '020102',
