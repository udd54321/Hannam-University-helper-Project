...
const ThirdFloorScreen = () => {
  ...
  const [currentImage] = useState(floors['11_3F'].image);
  ...
  const setStartPointer = (room) => {
    navigation.navigate('Gil', { roomId: room, startFloor: '11_3F', goalFloor: '11_3F' }); // startFloor와 goalFloor 전달
  };

  const setArrivalPointer = (room) => {
    navigation.navigate('Gil', { roomId: room, startFloor: '11_3F', goalFloor: '11_3F' });
  };
const excludedRooms = ['113FS1', '113FS2'];
  return (
    <SafeAreaProvider>
      <SafeAreaView style={styles.container}>
        ...
        >
          ...
          >
            ...
            {Object.keys(floors['11_3F'].rooms)
                           .filter((roomId) => !excludedRooms.includes(roomId)) // 제외할 방 필터링
                           .map((roomId) => {
              const room = floors['11_3F'].rooms[roomId];
              const isRotated = [
                '020101',
                '020101-A',
                '020102',
