...
const FirstFloorScreen = () => {
  ...
  const [currentImage] = useState(floors['11_1F'].image);
  ...
  const setStartPointer = (room) => {
    navigation.navigate('Gil', { roomId: room, startFloor: '11_1F', goalFloor: '11_1F' }); // startFloor와 goalFloor 전달
  };

  const setArrivalPointer = (room) => {
    navigation.navigate('Gil', { roomId: room, startFloor: '11_1F', goalFloor: '11_1F' });
  };
const excludedRooms = ['111FS1', '111FS2'];
  return (
    <SafeAreaProvider>
      <SafeAreaView style={styles.container}>
        ...
        >
          ...
          >
            ...
            {Object.keys(floors['11_1F'].rooms)
                           .filter((roomId) => !excludedRooms.includes(roomId)) // 제외할 방 필터링
                           .map((roomId) => {
              const room = floors['11_1F'].rooms[roomId];
              const isRotated = [
                '110101',
                '110102',
                '110107',
